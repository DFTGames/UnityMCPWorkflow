...
## Level settings
Each of the 8 levels has its own backdrop and palette; see [[Levels]]. The backdrop belongs to the level's
data, not to a scene, so changing a level's sky means changing its definition.

## Endless skies
An Endless run does not wear the campaign's backdrops. Those are landmarks, painted to tell one level from
another, and cutting between them every cycle announced that the mode is a loop and broke the run's continuity.

A run drifts instead through a **ring of six skies of its own**, each held for a while and then dissolved into
the next, the last leading back to the first. The ring runs on its own slow clock, deliberately unrelated to
the waves, the cycles and the boss, so the change never lines up with anything the player is watching.

The art is authored for blending, which is a requirement of the transition rather than a separate style choice:
soft and hazy, dark, **no focal point or landmark**, and matched to its neighbours in composition and value so
that only the hue really changes. Their brightness is normalised to one value, or the run would visibly pulse
as it drifts. A dissolve of zero seconds is rejected in code: the sky may never cut again.
