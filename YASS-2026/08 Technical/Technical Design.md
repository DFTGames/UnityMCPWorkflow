...
- **Assets are loaded by name, not referenced.** One scene plays everything, so a direct reference would put the whole game's asset graph behind it. The eight backdrops and six Endless skies (2048x1152, DXT1, 1.13 MB each resident, 15.8 MB for the set) live under `Resources/` and are loaded when needed and freed when not: a campaign level holds its own sky and nothing else, and an Endless run holds one sky, taking the next shortly before its dissolve and giving the old one back when it ends. The rule is `AssetResidency` in the rules layer; `ContentCache` performs it.
