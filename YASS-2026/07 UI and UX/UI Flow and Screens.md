...
- **Account:** asked once, on the way into the first run, then never again. A pilot name and a password, because the name is the account and that is what makes a score the player's rather than the machine's ([[Scoring]]). Three ways out: **Create account**, **Sign in**, and **Play without an account**, which is remembered. Whatever the service says comes back on one line above the buttons, in the player's words rather than the service's; that line keeps its place whether or not it says anything, so the buttons do not move under the player's hand.
