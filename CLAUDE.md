...
4. Read `Temp/YASS-TestResults-EditMode.txt` / `-PlayMode.txt`, which `TestResultsWriter.cs` writes after every run. Runs are asynchronous: poll the file's timestamp rather than trusting stale contents (EditMode takes a few seconds, PlayMode about 45 s). `Unity_ManageEditor` with `action: GetState` shows whether play mode is still running.
...
- `Tools/YASS/Build Feedback Assets` generates the sound bank and the particle prefabs from code (tuning lives in `FeedbackBuilder`); `Tools/YASS/Build Audio Mixer` creates the mixer; `Tools/YASS/Build Feedback Wiring` puts the feedback objects into both scenes and adds `HitFlash` to the damageable prefabs. Sound effects are AI-generated into `Assets/_Game/Audio/SFX/`, one file per `Sfx` entry, named after it; music into `Assets/_Game/Audio/Music/`.
- **Unity has no public API for creating an audio mixer**, its groups or its exposed parameters, so `AudioMixerBuilder` drives the internal `UnityEditor.Audio.AudioMixerController` by reflection. It is written to fail loudly if those names move; the mixer can then be made by hand.
- Any builder that saves a scene containing menus must lay its canvases out first (`MenuSceneParts.SaveAndRelease` does this): a screen that is already visible when the scene loads has nothing to trigger a layout pass, so the saved positions are what the player sees on the first frame.
...
`Assets/_Game/Scenes/Endless.unity` is one scene that keeps going: cycles of ten waves and a boss, each faster, fuller and worth more (GDD "Core Loop", Endless mode). It authors no content, drawing each cycle from the campaign's levels through `EndlessDefinition` (`ScriptableObjects/Levels/Endless.asset`).
...
- **A scene that carries an `EndlessDefinition` is an Endless run; one that carries a `LevelDefinition` is a campaign level.** The scene declares what it is, so either opens directly in the editor, and `HasValidSetup` fails loudly if a scene has both or disagrees with the mode the flow layer started.
...
- `Tools/YASS/Build Endless` generates the definition and the scene (a copy of Level 1's, re-pointed), and adds the HUD's cycle label. `MenuSceneParts.PlayableScenePaths()` covers the eight levels **and** Endless, so the menu and feedback builders cannot leave it behind.
...
A campaign run is `CampaignRun` (Core) held by `RunContext`: it banks each cleared level's score and what each player carries (lives, health, weapon), and `GameRunner` restores that when the next level loads. The levels themselves are listed in `Assets/_Game/Resources/Campaign.asset` (a `Campaign`, loaded by path so the flow layer needs no scene reference); add a level by adding its scene there and to the build settings. `CampaignProgress` saves how far each difficulty has got through the same key-value store as the settings.

**`Tools/YASS/Build Title Scene` rebuilds the title from an empty scene**, so everything another builder had put there goes with it. It calls the feedback wiring itself to put the music and UI sounds back; anything else added to that scene in future has to be restored the same way. `SceneAudioTests` fails if any playable scene loses its audio, because a silent scene looks exactly like a working one.

**The menu screens are generated, not hand-authored.** `Tools/YASS/Build Title Scene` and `Tools/YASS/Build Level Menus` (`Scripts/Editor/TitleSceneBuilder.cs`, `LevelMenuBuilder.cs`, widgets and palette in `MenuUiFactory.cs`) rebuild them from code, so edits made by hand inside `Canvas`/`MenuCanvas` or the `Menus` object are lost on the next run: change the builder instead, then re-run it. The title scene is build index 0 and `Level01` is 1.

- **The campaign's content is generated, not hand-authored.** `Tools/YASS/Build Enemies` builds the enemy definitions and prefabs, `Tools/YASS/Build Bosses` the eight bosses (from `Prefabs/_BossTemplate.prefab`, which is never itself an output), and `Tools/YASS/Build Levels` the wave scripts for levels 2 to 8, their scenes, the campaign list and the build settings. **`Level01.unity` is the only level scene anybody edits**: the other seven are deleted and re-copied from it on every run, so a change made to one of them is lost. Change Level 1, or the builder, and rebuild.
...
- Build scenes: `Assets/_Game/Scenes/Level01.unity` (index 0; formerly the Prototype scene). The template's `SampleScene` is kept but disabled.
- Sprites: gameplay sprites live in `Sprites/Ships`, `Sprites/Hazards`, `Sprites/Pickups`, `Sprites/Bosses` and `Sprites/Procedural`, all packed into `Sprites/Gameplay.spriteatlasv2`. Keep those sources uncompressed with dimensions divisible by 4 (the atlas compresses). Sorting orders are listed in the GDD's Art Direction page.
