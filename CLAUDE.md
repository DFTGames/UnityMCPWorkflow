...
- `Tools/YASS/Build Feedback Assets` generates the sound bank and the particle prefabs from code (tuning lives in `FeedbackBuilder`); `Tools/YASS/Build Feedback Wiring` puts the feedback objects into both scenes and adds `HitFlash` to the damageable prefabs. Sound effects are AI-generated into `Assets/_Game/Audio/SFX/`, one file per `Sfx` entry, named after it.
