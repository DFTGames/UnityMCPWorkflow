...
- `Tools/YASS/Build Feedback Assets` generates the sound bank and the particle prefabs from code (tuning lives in `FeedbackBuilder`) and `Tools/YASS/Build Audio Mixer` creates the mixer. Both make **assets**, not scenes: the feedback objects now live in the two scenes and are edited there like anything else. Sound effects are AI-generated into `Assets/_Game/Audio/SFX/`, one file per `Sfx` entry, named after it; music into `Assets/_Game/Audio/Music/`.
...
- Still to do: boss and enemy prefabs are ordinary references, so a cycle still holds every boss and enemy the campaign has. Same treatment, not yet applied.
...
- **Enemies and bosses are generated from tables; levels are not.** `Tools/YASS/Build Enemies` builds the enemy definitions and prefabs and `Tools/YASS/Build Bosses` the eight bosses (from `Prefabs/_BossTemplate.prefab`, which is never itself an output). The levels themselves are plain data: edit `ScriptableObjects/Levels/LevelNN.asset` in the Inspector to change a wave, a backdrop or a boss. There is no level builder and no level scene to rebuild.
...
- Sprites: gameplay sprites live in `Sprites/Ships`, `Sprites/Hazards`, `Sprites/Pickups`, `Sprites/Bosses` and `Sprites/Procedural`, all packed into `Sprites/Gameplay.spriteatlasv2`. Backdrops and skies are **not** in the atlas: they are in `Resources/Backgrounds/` and `Resources/Skies/`, loaded by path. Keep them out of the atlas, or freeing one would destroy the shared atlas page (`ContentCache` refuses and says so). Keep those sources uncompressed with dimensions divisible by 4 (the atlas compresses). Sorting orders are listed in the GDD's Art Direction page.
